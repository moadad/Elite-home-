...
import { getFirestore, collection, doc, setDoc, addDoc, updateDoc, deleteDoc, writeBatch, onSnapshot, getDocs, serverTimestamp, query, where, limit } from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js';
...
const ADMIN_EMAILS = ['admin@elite.home'];
const CLOUDINARY_CLOUD_NAME = 'dueuyivqo';
const CART_STORAGE_KEY = 'elite_home_ware_cart_v1';
const PRODUCT_PAGE_SIZE = 24;
const APP_SW_VERSION = 'elite-home-ware-v5';
const APP_ROOT_URL = new URL('./', window.location.href);
const SITE_URL = APP_ROOT_URL.href;
const OPEN_CART_FLAG_KEY = 'elite_home_open_cart_after_nav';
const SITE_NAME_AR = 'إيليت هوم وير';
const SITE_NAME_EN = 'Elite Home Ware';
const DEFAULT_COUNTRY_DIAL_CODE = '963';
const DEFAULT_COUNTRY_PHONE_PLACEHOLDER = '+9639XXXXXXXX';

const DEFAULT_PAYMENT_POLICY = `الدفع المتاح حالياً: كاش فقط.
يتم تثبيت وسائل الدفع الإضافية لاحقاً من لوحة التحكم.
لا يتم اعتماد الطلب إلا بعد مراجعته من الإدارة.`;

const DEFAULT_RETURN_POLICY = `يمكن طلب الاستبدال أو الإرجاع حسب سياسة المتجر وبعد مراجعة حالة المنتج.
المنتجات المباعة في العروض أو التي تم استخدامها لا يشملها الاستبدال إلا وفق موافقة الإدارة.
تحتفظ الإدارة بحق مراجعة كل حالة بشكل منفصل قبل اعتمادها.`;

const DEFAULT_SHIPPING_POLICY = `الاستلام حالياً من أحد فروعنا فقط.
سيتم إتاحة أنواع الشحن لاحقاً من لوحة التحكم عند التفعيل.
يرجى اختيار الفرع المناسب أثناء إتمام الطلب.`;

const DEFAULT_TERMS_POLICY = `يمكن البيع بالقطعة أو بالدستة أو بالاثنين معاً حسب كل منتج.
الأسعار والخصومات قابلة للتحديث من لوحة التحكم.
تأكيد الطلب مرتبط بتوفر المنتج وقت المراجعة.
يحق للإدارة تعديل الواجهة والمنتجات والسياسات والفروع من لوحة التحكم.`;

const DEFAULT_PAYMENT_METHODS = [
  { key: 'cash', label: 'كاش فقط', icon: 'fa-solid fa-money-bill-wave' },
];

const DEFAULT_STOREFRONT = {
  companyName: 'Elite Home Ware',
  tagline: 'HOMEWARE & KITCHEN',
  heroTitle: 'تشكيلة Elite Home Ware',
  heroSubtitle: 'منتجات مرتبة حسب التصنيف الرئيسي والتصنيف الفرعي مع تجربة شراء فخمة وسريعة.',
  heroBadge: 'قطعة • دستة • استلام فروع',
  logoUrl: '',
  accentColor: '#C9A45D',
  accentColor2: '#0B493E',
  featuredLimit: 8,
...
const DEFAULT_COMPANY = {
  ...
  factoryMap: '',
  shopMap: '',
  cloudinaryPreset: 'Unsigned',
  about: 'متجر Elite home ware لعرض وبيع الأدوات المنزلية ومنتجات الهوم وير بطريقة أنيقة وسهلة مع إمكانية البيع بالقطعة أو بالدستة.',
};
...
const el = {
  ...
  branchesInput: id('branchesInput'),
  ...
  quickPackQtyInput: id('quickPackQtyInput'),
  quickPieceQtyInput: id('quickPieceQtyInput'),
  ...
  customerBranchInput: id('customerBranchInput'),
  ...
  trackOrderRefInput: id('trackOrderRefInput'),
  trackOrderPhoneInput: id('trackOrderPhoneInput'),
  trackOrderSearchBtn: id('trackOrderSearchBtn'),
  trackOrderResult: id('trackOrderResult'),
...
function bindUI() {
  ...
  el.brandTrigger.addEventListener('click', handleAdminTap);
  ...
  el.trackOrderBtn?.addEventListener('click', () => openModal('trackOrderModal'));
  el.menuTrackOrderBtn?.addEventListener('click', () => { closeDrawers(); openModal('trackOrderModal'); });
  el.trackOrderSearchBtn?.addEventListener('click', searchOrderStatus);

  [el.trackOrderRefInput, el.trackOrderPhoneInput].filter(Boolean).forEach((input) => {
  ...
  bindSyrianPhoneField(el.companyPhoneInput);
  bindSyrianPhoneField(el.companyPhone2Input);
  bindSyrianPhoneField(el.companyWhatsappInput);
  bindSyrianPhoneField(el.customerPhoneInput);
  bindSyrianPhoneField(el.trackOrderPhoneInput);
  ...
  [el.contactModal, el.policyModal, el.imageModal, el.checkoutModal, el.invoiceModal, el.quickProductModal, el.trackOrderModal].filter(Boolean).forEach((modal) => {
    ['click', 'pointerup', 'touchend'].forEach((eventName) => {
  ...
  el.quickProductAddBtn?.addEventListener('click', (e) => {
    ...
    if (!productId) return;
    const packQty = toInt(el.quickPackQtyInput?.value || 0);
    const pieceQty = toInt(el.quickPieceQtyInput?.value || 0);
    addToCart(productId, { packQty, pieceQty });
  });
...
function renderCheckoutSelectors() {
  ...
  if (el.customerBranchInput) {
    const currentBranch = el.customerBranchInput.value;
    el.customerBranchInput.innerHTML = branches.map((branch) => `<option value="${escapeAttr(branch)}">${escapeHTML(branch)}</option>`).join('');
    el.customerBranchInput.value = branches.includes(currentBranch) ? currentBranch : (branches[0] || 'الفرع الرئيسي');
  }
  if (el.paymentMethodInput) {
    el.paymentMethodInput.innerHTML = '<option value="كاش فقط">كاش فقط</option>';
    el.paymentMethodInput.value = 'كاش فقط';
  }
  if (el.shippingMethodInput) {
    el.shippingMethodInput.innerHTML = '<option value="استلام من أحد فروعنا">استلام من أحد فروعنا</option>';
    el.shippingMethodInput.value = 'استلام من أحد فروعنا';
  }
}

function getBranchOptions() {
  const saved = Array.isArray(state.storeSettings?.branches) ? state.storeSettings.branches : [];
  const clean = [...new Set(saved.map((item) => String(item || '').trim()).filter(Boolean))];
  return clean.length ? clean : ['الفرع الرئيسي'];
}
...
function syncCatalogHero() {
  ...
  if (step === 'seasons') {
    title = 'المواسم';
    subtitle = 'اختر المجموعة لبدء التصفح داخل المتجر.';
    badge = 'واجهة المتجر';
    sectionTitle = 'المنتجات';
  } else if (step === 'prefixes') {
    title = selectedSeason || defaultTitle;
    subtitle = 'اختر التصنيف الرئيسي أو اعرض كل موديلات المجموعة.';
    badge = 'البادئات';
    ...
    else if (selectedSeason) subtitle = `كل موديلات ${selectedSeason} مع تصنيف التصنيف الرئيسي`;
    else subtitle = defaultSubtitle;
    badge = subCategoryLabel ? 'التصنيف الداخلي' : (categoryLabel ? `بادئة ${selectedCategory}` : (selectedSeason ? 'كل منتجات المجموعة' : defaultBadge));
...
function renderCatalogFlow() {
  ...
  if (step === 'seasons') {
      ...
      <div class="catalog-browser-card">
        <div class="catalog-browser-head">
          <div>
            ...
            <h2>اختر المجموعة</h2>
            <p>ابدأ من المجموعة ثم انتقل إلى البادئات ثم الموديلات.</p>
          </div>
        </div>
      ...
      <div class="catalog-browser-card">
        ...
        <div class="catalog-browser-head catalog-browser-head--split">
          <div>
            ...
            <h2>اختر التصنيف الرئيسي</h2>
            <p>يمكنك عرض كل موديلات المجموعة مباشرة أو الدخول إلى بادئة محددة.</p>
          </div>
          <button class="catalog-all-btn" data-action="season-all" data-season="${escapeAttr(selectedSeason)}">
            <i class="fa-solid fa-layer-group"></i>
            <span>كل منتجات المجموعة</span>
          </button>
        </div>
        <div class="catalog-browser-grid prefix-grid">${prefixCards || '<div class="catalog-empty-inline">لا توجد بادئات لهذا المجموعة بعد.</div>'}</div>
      </div>`;
  } else {
    ...
    const description = selectedCategory === 'all'
      ? 'جميع منتجات المجموعة مع إمكانية التصفية حسب التصنيف الرئيسي.'
    ...
    html = `
      <div class="catalog-browser-card">
        ...
        <div class="prefix-filters-stack">
          ...
          ${subCategoryChips ? `<div class="sub-category-panel"><div class="sub-category-label">التصنيف الفرعي</div><div class="prefix-filters-bar">${subCategoryChips}</div></div>` : ''}
        </div>
      </div>`;
  }
  ...
  renderedItems.forEach((product, index) => {
    ...
    card.innerHTML = `
      ...
      <div class="product-body">
        ...
        <div class="muted product-pack-price">سعر الدستة <strong class="pack-price-value"></strong></div>
        <div class="card-actions single-action">
          <button class="primary-btn add-btn" aria-label="اختر كمية ${escapeAttr(productLabel)}"><i class="fa-solid fa-cart-plus"></i><span>اختر الكمية</span></button>
        </div>
      </div>`;
    card.querySelector('.product-title').textContent = product.name || 'بدون اسم';
    card.querySelector('.product-model-line').textContent = [product.model ? `موديل ${product.model}` : '', getCodeCategoryLabel(product.codeCategory), getProductSubCategory(product)].filter(Boolean).join(' • ');
    card.querySelector('.piece-price-value').textContent = formatCurrency(getPiecePrice(product));
    card.querySelector('.pack-price-value').textContent = formatCurrency(getDisplayPrice(product));
...
function buildProductDescription(product) {
  ...
  const parts = [
    ...
    product?.season ? `المجموعة: ${product.season}` : '',
    getCodeCategoryLabel(product?.codeCategory) ? `التصنيف الرئيسي: ${getCodeCategoryLabel(product.codeCategory)}` : '',
...
function syncSiteMeta() {
  ...
  const title = `${brand} | أدوات منزلية وهوم وير`;
  const description = `${brand} لهوم وير ومنتجات منزلية. ${visibleCount.toLocaleString('en-US')} موديل متاح مع صور المنتجات والتواصل والطلب.`;
...
function renderSeoCatalog() {
  ...
  const brand = getBrandDisplayName();
  const lines = [
    `<h2>${escapeHTML(brand)} - فهرس موديلات هوم وير ومنتجات منزلية</h2>`,
    `<p>${escapeHTML(brand)} يقدم ${visibleProducts.length.toLocaleString('en-US')} موديل من الأدوات المنزلية والهوم وير.</p>`
  ];
  ...
  const graph = [
    {
      ...
      description: `${brand} لهوم وير ومنتجات منزلية`,
    ...
    ...topProducts.map((product) => ({
      ...
      category: getCodeCategoryLabel(product.codeCategory) || 'هوم وير',
...
function renderCart() {
  ...
  const count = state.cart.reduce((sum, item) => sum + getSeriesCount(item) + getLoosePieceCount(item), 0);
  el.cartCount.textContent = String(count);
  el.cartItemsCount.textContent = count ? `${count} اختيار` : '0 اختيار';
  ...
  state.cart.forEach((item) => {
    ...
    card.className = 'cart-item cart-item-rich';
    card.innerHTML = `
      ...
      <div class="cart-item-main">
        ...
        <div class="muted">${escapeHTML(getCodeCategoryLabel(item.codeCategory))}${getProductSubCategory(item) ? ` • ${escapeHTML(getProductSubCategory(item))}` : ''}</div>
        <div class="muted">${escapeHTML(getSeriesLabel(item))}</div>
        <div class="muted">سعر القطعة ${formatCurrency(getPiecePrice(item))}</div>
        <strong>سعر الدستة ${formatCurrency(getSeriesValue(item))}</strong>
      </div>
      <div class="cart-qty-stack">
        <div class="cart-qty-group">
          <span>دست</span>
          <div class="qty-box dual">
            <button data-action="pack-plus">+</button>
            <span>${getSeriesCount(item)}</span>
            <button data-action="pack-minus">-</button>
          </div>
        </div>
        <div class="cart-qty-group">
          <span>قطع</span>
          <div class="qty-box dual">
            <button data-action="piece-plus">+</button>
            <span>${getLoosePieceCount(item)}</span>
            <button data-action="piece-minus">-</button>
          </div>
        </div>
        <div class="cart-line-total">${formatCurrency(getLineTotal(item))}</div>
      </div>`;

    card.querySelector('[data-action="pack-plus"]').addEventListener('click', () => changeCartQty(item.id, 'pack', 1));
    card.querySelector('[data-action="pack-minus"]').addEventListener('click', () => changeCartQty(item.id, 'pack', -1));
    card.querySelector('[data-action="piece-plus"]').addEventListener('click', () => changeCartQty(item.id, 'piece', 1));
    card.querySelector('[data-action="piece-minus"]').addEventListener('click', () => changeCartQty(item.id, 'piece', -1));
...
function renderAdminForms() {
  ...
  [el.companyPhoneInput, el.companyPhone2Input, el.companyWhatsappInput, el.customerPhoneInput, el.trackOrderPhoneInput].filter(Boolean).forEach((input) => {
    input.placeholder = DEFAULT_COUNTRY_PHONE_PLACEHOLDER;
  });
  ...
  el.branchesInput.value = getBranchOptions().join(', ');
...
function rebuildSeasonOptions() {
  ...
  el.productSeasonInput.value = options.includes(current) ? current : (options[0] || 'الرئيسية');
}
  ...
  products.forEach((product) => {
    ...
    item.innerHTML = `
      ...
      <div>
        ...
        <div class="muted">${hasDiscount(product) ? `خصم ${Math.round(toNumber(product.discountPercent))}% • ` : ''}سعر القطعة ${formatCurrency(getPiecePrice(product))} • سعر الدستة ${formatCurrency(getDisplayPrice(product))} • ${escapeHTML(getSeriesLabel(product))}</div>
      </div>
  ...
  state.orders.forEach((order) => {
    ...
    card.innerHTML = `
      ...
      <div class="order-overview-grid">
        ...
        <div class="order-overview-card"><span>الفرع</span><strong>${escapeHTML(order.branchName || '—')}</strong></div>
        <div class="order-overview-card"><span>الدفع</span><strong>${escapeHTML(order.paymentMethod || '—')}</strong></div>
      </div>
...
async function saveCompanyData() {
  ...
  try {
    await setDoc(doc(db, 'company', 'main'), {
      companyName: el.companyNameInput.value.trim(),
      tagline: el.companyTaglineInput.value.trim(),
      phone1: formatPhoneForDisplay(el.companyPhoneInput.value),
      phone2: formatPhoneForDisplay(el.companyPhone2Input.value),
      whatsapp: formatPhoneForDisplay(el.companyWhatsappInput.value),
      ...
      cloudinaryPreset: el.cloudinaryPresetInput.value.trim() || 'Unsigned',
      about: el.companyAboutInput.value.trim(),
      updatedAt: serverTimestamp(),
    }, { merge: true });
    await setDoc(doc(db, 'settings', 'store'), {
      branches: parseCommaList(el.branchesInput.value),
      updatedAt: serverTimestamp(),
    }, { merge: true });
...
async function saveSeasons() {
  ...
  try {
    ...
    renderCheckoutSelectors();
    showToast('تم حفظ المجموعات');
  } catch (error) {
    console.error(error);
    showToast('تعذر حفظ المجموعات');
  }
}
...
async function saveCodeCategoryLabel(code, label) {
  ...
  try {
    ...
    const payload = { type: 'code', code: cleanCode, label: cleanLabel, updatedAt: serverTimestamp() };
...
async function uploadFilesToCloudinary(files, folder) {
  const preset = (state.company.cloudinaryPreset || el.cloudinaryPresetInput.value || 'Unsigned').trim();
  const urls = [];
  for (const file of files) {
    ...
    body.append('folder', `elite-home-ware/${folder}`);
...
async function saveProduct() {
  ...
  const season = el.productSeasonInput.value || getSeasonOptions()[0] || 'الرئيسية';
  const discountPercent = clamp(toNumber(el.productDiscountInput.value), 0, 99);
  const seriesQtyText = el.productMinQtyInput.value.trim() || '12';
  const seriesQtyNumber = getSeriesQtyNumber({ seriesQtyText });
  const priceWholesale = round2(pricePiece * seriesQtyNumber);
  const mainCategory = normalizeCategoryCode(el.productMainCategoryInput.value);
  if (!name || !model || rawPricePiece === '' || !seriesQtyText || !mainCategory) return showToast('الاسم والموديل وسعر القطعة والتصنيف الرئيسي والعدد داخل الدستة مطلوبة');
  const payload = {
    name,
    model,
    pricePiece,
...
function populateProductForm(product) {
  ...
  el.productSeasonInput.value = product.season || getSeasonOptions()[0] || 'الرئيسية';
  el.productMainCategoryInput.value = String(product.codeCategory || '').trim();
...
function resetProductForm() {
  ...
  el.productSeasonInput.value = getSeasonOptions()[0] || 'الرئيسية';
  el.productMainCategoryInput.value = '';
  el.productSubCategoryInput.value = '';
  el.productSizesInput.value = '';
  el.productMinQtyInput.value = '12';
...
async function addToCart(productId, options = {}) {
  ...
  if (!product) return;
  const packQty = Math.max(0, toInt(options.packQty ?? 1));
  const pieceQty = Math.max(0, toInt(options.pieceQty ?? 0));
  if (packQty <= 0 && pieceQty <= 0) return showToast('اختر كمية صحيحة');
  const existing = state.cart.find((item) => item.id === productId);
  if (existing) {
    existing.packQty = getSeriesCount(existing) + packQty;
    existing.looseQty = getLoosePieceCount(existing) + pieceQty;
    existing.unitPrice = getDisplayPrice(product);
    existing.pricePiece = getPiecePrice(product);
    existing.originalPrice = getSeriesBasePrice(product);
    existing.seriesQtyText = getSeriesQtyText(product);
  } else {
    state.cart.push({
      id: product.id,
      name: product.name,
...
function changeCartQty(productId, field, delta) {
  const item = state.cart.find((entry) => entry.id === productId);
  if (!item) return;
  if (field === 'pack') item.packQty = Math.max(0, getSeriesCount(item) + delta);
  if (field === 'piece') item.looseQty = Math.max(0, getLoosePieceCount(item) + delta);
  item.qty = item.packQty;
  if (getSeriesCount(item) <= 0 && getLoosePieceCount(item) <= 0) {
...
async function submitOrder() {
  ...
  const customerPhoneRaw = el.customerPhoneInput.value.trim();
  const customerPhoneDigits = normalizeSyrianPhoneNumber(customerPhoneRaw);
  const customerPhone = formatPhoneForDisplay(customerPhoneRaw);
  const city = el.customerCityInput.value.trim();
  const address = el.customerAddressInput.value.trim();
  const branchName = el.customerBranchInput?.value || getBranchOptions()[0] || 'الفرع الرئيسي';
  const paymentMethod = el.paymentMethodInput.value || 'كاش فقط';
  const shippingMethod = el.shippingMethodInput.value || 'استلام من أحد فروعنا';
  const notes = el.customerNotesInput.value.trim();

  if (!customerName || !customerPhoneRaw || !city || !address) return showToast('أكمل بيانات العميل');
  if (!customerPhoneDigits) return showToast('أدخل رقم هاتف صحيح يبدأ بـ +963 أو 09');
  if (!state.cart.length) return showToast('السلة فارغة');
  ...
  const items = state.cart.map((item) => ({
    ...
    packQty: getSeriesCount(item),
    looseQty: getLoosePieceCount(item),
  ...
  try {
    ...
    if (el.customerBranchInput) el.customerBranchInput.value = getBranchOptions()[0] || 'الفرع الرئيسي';
...
function exportProductsExcel() {
  const rows = state.products.map((product) => ({ name: product.name || '', model: product.model || '', mainCategory: String(product.codeCategory || '').trim(), subCategory: getProductSubCategory(product), collection: product.season || '', pricePiece: getPiecePrice(product), priceWholesale: getSeriesBasePrice(product), discountPercent: toNumber(product.discountPercent || 0), sizes: product.sizes || '', seriesQtyText: getSeriesQtyText(product), badgeText: product.badgeText || '', pinned: Boolean(product.pinned), visible: product.visible !== false, description: product.description || '', imageUrls: normalizeImageUrls(product.imageUrls).join('\n') }));
...
async function importProductsExcel(event) {
  ...
  try {
    ...
    rows.forEach((row) => {
      ...
      const mainCategory = normalizeCategoryCode(firstValue(row, ['mainCategory', 'codeCategory', 'category', 'التصنيف الرئيسي', 'التصنيف']));
      const seriesQtyText = String(firstValue(row, ['seriesQtyText', 'seriesQty', 'كمية الدستة', 'كمية السيري', 'عدد القطع في الدستة', 'السيري', 'minQty']) || '12').trim();
      const seriesQtyNumber = getSeriesQtyNumber({ seriesQtyText });
      const piecePriceRaw = firstValue(row, ['pricePiece', 'piecePrice', 'سعر القطعة']);
      const seriesPriceRaw = firstValue(row, ['priceWholesale', 'priceSeries', 'price', 'سعر الجملة', 'سعر الدستة', 'سعر الدستة']);
      ...
      if (!name || !model || !mainCategory || Number.isNaN(pricePiece) || Number.isNaN(priceWholesale)) return;
      ...
      batch.set(ref, {
        ...
        season: String(firstValue(row, ['collection', 'season', 'المجموعة', 'المجموعة']) || getSeasonOptions()[0] || 'الرئيسية').trim(),
        codeCategory: mainCategory,
        subCategory: normalizeSubCategory(firstValue(row, ['subCategory', 'subcategory', 'groupLabel', 'التصنيف الفرعي', 'الفئة'])),
...
async function searchOrderStatus() {
  const ref = String(el.trackOrderRefInput?.value || '').trim();
  const phoneRaw = String(el.trackOrderPhoneInput?.value || '').trim();
  const phone = normalizeSyrianPhoneNumber(phoneRaw);
  if (!ref || !phoneRaw) {
    el.trackOrderResult.innerHTML = '<div class="muted">أدخل رقم الطلب ورقم الهاتف.</div>';
    return;
  }
  el.trackOrderSearchBtn.disabled = true;
  el.trackOrderResult.innerHTML = '<div class="muted">جاري البحث...</div>';
  try {
    const snapshot = await getDocs(query(collection(db, 'orders'), where('orderNo', '==', ref), limit(3)));
    const matchDoc = snapshot.docs.find((docItem) => normalizeSyrianPhoneNumber(docItem.data().customerPhone || '') === phone);
    if (!matchDoc) {
      el.trackOrderResult.innerHTML = '<div class="muted">لم يتم العثور على طلب مطابق.</div>';
      return;
    }
    const order = { id: matchDoc.id, ...matchDoc.data() };
    const totals = getOrderTotals(order);
    el.trackOrderResult.innerHTML = `
      <div class="track-order-card">
        <div class="track-order-head">
          <strong>${escapeHTML(getOrderReference(order))}</strong>
          <span class="badge soft">${escapeHTML(order.status || 'جديد')}</span>
        </div>
        <div class="track-order-grid">
          <div><span>العميل</span><strong>${escapeHTML(order.customerName || '—')}</strong></div>
          <div><span>الفرع</span><strong>${escapeHTML(order.branchName || '—')}</strong></div>
          <div><span>الإجمالي</span><strong>${escapeHTML(formatCurrency(totals.total))}</strong></div>
          <div><span>التاريخ</span><strong>${escapeHTML(formatOrderDate(order))}</strong></div>
        </div>
      </div>`;
  } catch (error) {
    console.error(error);
    el.trackOrderResult.innerHTML = '<div class="muted">تعذر جلب حالة الطلب الآن.</div>';
  } finally {
    el.trackOrderSearchBtn.disabled = false;
  }
}
...
function openQuickProduct(product) {
  ...
  el.quickProductPrice.textContent = `${formatCurrency(getPiecePrice(product))} / القطعة`;
  el.quickProductSeries.textContent = `${formatCurrency(getDisplayPrice(product))} / الدستة`;
  ...
  if (el.quickPackQtyInput) el.quickPackQtyInput.value = '1';
  if (el.quickPieceQtyInput) el.quickPieceQtyInput.value = '0';
...
function getActionErrorMessage(error, fallback = 'تعذر تنفيذ العملية') {
  ...
  if (code.includes('permission-denied') || message.includes('Missing or insufficient permissions')) {
    return 'لا توجد صلاحية للحفظ من Firebase. ارفع قواعد Firestore الجديدة ثم سجّل خروج/دخول بحساب الأدمن وأعد المحاولة';
  }
...
function getSeriesCount(item) {
  const value = toInt(item?.packQty ?? item?.qty ?? item?.seriesCount ?? 0);
  return value > 0 ? value : 0;
}

function getLoosePieceCount(item) {
  const value = toInt(item?.looseQty ?? item?.pieceOnlyQty ?? item?.pieceSingleQty ?? 0);
  return value > 0 ? value : 0;
}

function getPiecesPerSeries(item) {
  const explicit = toInt(item?.piecesPerSeries ?? item?.pieceQtyPerSeries ?? item?.packQtyPerSet ?? item?.packQtySize ?? item?.packQty ?? 0);
...
function getPieceQuantity(item) {
  return (getSeriesCount(item) * getPiecesPerSeries(item)) + getLoosePieceCount(item);
}
...
function getLineTotal(item) {
  return round2((getSeriesCount(item) * getSeriesValue(item)) + (getLoosePieceCount(item) * getPiecePrice(item)));
}
...
function getOrderTotals(order) {
  return (Array.isArray(order?.items) ? order.items : []).reduce((acc, item) => {
    ...
    acc.loosePieces += getLoosePieceCount(item);
    acc.pieces += getLinePiecesTotal(item);
    acc.total += getLineTotal(item);
    return acc;
  }, { models: 0, series: 0, loosePieces: 0, pieces: 0, total: 0 });
}
...
function buildWhatsAppOrderMessage(order) {
  ...
  (order.items || []).forEach((item, index) => {
    lines.push(
      ...
      `عدد الدست: ${getSeriesCount(item)}`,
      `القطع المفردة: ${getLoosePieceCount(item)}`,
      `إجمالي القطع: ${getPieceQuantity(item)}`,
      separator
    );
  });
...
async function buildInvoiceCanvas(order) {
  ...
  const cols = [
    ...
    { key: 'seriesCount', title: 'عدد الدست', width: 148, align: 'center' },
    { key: 'pieceQty', title: 'كمية القطع', width: 150, align: 'center' },
    { key: 'seriesValue', title: 'قيمة الدستة', width: 190, align: 'center' },
...
function drawInvoiceSummary(ctx, { width, margin, contentWidth, y, totals, order, summaryHeight }) {
  ...
  const summaryCards = [
    ...
    { title: 'عدد الدست', value: totals.series },
  ...
  ctx.font = '700 24px Cairo, sans-serif';
  ctx.fillText('تشمل قيمة الدستة والقطع المفردة والإجمالي النهائي', totalX + totalCardWidth - 28, y + 164);
}
...
function getCodeCategoryLabel(code) {
  ...
  return match?.label || fallbackLabel || safeCode;
}
...
function deriveCodeCategory(modelValue) {
  return normalizeCategoryCode(modelValue) || 'عام';
}
...
function initials(text) { return String(text || 'EH').trim().split(/\s+/).slice(0, 2).map((item) => item[0] || '').join('').toUpperCase() || 'EH'; }
...
const formatCurrency = (value) => `${round2(toNumber(value || 0)).toLocaleString('en-US')} ل.س`;
...
function normalizeSyrianPhoneNumber(value) { let digits = String(value || '').replace(/\D/g, ''); if (!digits) return ''; if (digits.startsWith('00')) digits = digits.slice(2); if (digits.startsWith(DEFAULT_COUNTRY_DIAL_CODE)) return digits; if (digits.startsWith('0')) return `${DEFAULT_COUNTRY_DIAL_CODE}${digits.slice(1)}`; if (digits.startsWith('9')) return `${DEFAULT_COUNTRY_DIAL_CODE}${digits}`; return ''; }
function formatPhoneForDisplay(value) { const digits = normalizeSyrianPhoneNumber(value); return digits ? `+${digits}` : String(value || '').trim(); }
function bindSyrianPhoneField(input) { if (!input || input.dataset.phoneBound === 'true') return; input.dataset.phoneBound = 'true'; input.placeholder = DEFAULT_COUNTRY_PHONE_PLACEHOLDER; input.addEventListener('blur', () => { input.value = formatPhoneForDisplay(input.value); }); }
function normalizeWhatsAppNumber(value) { return normalizeSyrianPhoneNumber(value); }
