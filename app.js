...
const APP_SW_VERSION = 'elitehome-fast-secure-v2';
...
const DEFAULT_STOREFRONT = {
  ...
  heroTitle: 'مجموعات منزلية مختارة',
  heroSubtitle: 'عرض فاخر ومنظم للأدوات المنزلية مع دعم البيع بالقطعة والدستة.',
  heroBadge: 'قطعة • دستة',
  logoUrl: '',
  accentColor: '#0f766e',
  accentColor2: '#0f172a',
...
const DEFAULT_STORE_SETTINGS = {
  ...
  seasons: ['المطبخ', 'الضيافة والتقديم', 'التخزين والتنظيم', 'التنظيف'],
  codeCategoryLabels: {},
};
...
function renderStorefront() {
  ...
  el.seasonsCounter.textContent = getSubCategoryKeys().length.toLocaleString('en-US');
...
function renderMenu() {
  const selectedMainCategory = state.filter.season;
  const selectedSubCategory = state.filter.subCategory;
  const menuSubCategories = getSubCategoryKeys({ season: selectedMainCategory === 'all' ? '' : selectedMainCategory, category: 'all' });
  el.menuCategoryList.innerHTML = '';
  el.menuCategoryList.appendChild(makeFilterChip('كل الأقسام', selectedMainCategory === 'all', () => {
    goToSeasonsView({ closeAfter: true });
  }));
  getSeasonOptions().forEach((section) => {
    el.menuCategoryList.appendChild(makeFilterChip(section, selectedMainCategory === section, () => {
      activateCatalogProducts(section, 'all', { closeAfter: true, preserveSort: true, subCategory: 'all' });
    }));
  });

  el.menuSeasonList.innerHTML = '';
  el.menuSeasonList.appendChild(makeFilterChip('كل التصنيفات', selectedSubCategory === 'all', () => {
    if (selectedMainCategory !== 'all') activateCatalogProducts(selectedMainCategory, 'all', { closeAfter: true, preserveSort: true, subCategory: 'all' });
    else goToSeasonsView({ closeAfter: true });
  }));
  menuSubCategories.forEach((label) => {
    el.menuSeasonList.appendChild(makeFilterChip(label, selectedSubCategory === label, () => {
      activateCatalogProducts(selectedMainCategory === 'all' ? '' : selectedMainCategory, 'all', { closeAfter: true, preserveSort: true, subCategory: label });
    }));
  });
...
function applyFilters() {
  ...
  let items = getVisibleProducts().filter((product) => {
    const haystack = `${product._searchText || ''} ${product.season || ''} ${getProductSubCategory(product)}`.toLowerCase();
...
function buildProductAlt(product) {
  ...
  const bits = [
    ...
    product?.season ? `القسم ${product.season}` : '',
...
function buildProductDescription(product) {
  ...
  const parts = [
    ...
    product?.season ? `القسم الرئيسي: ${product.season}` : '',
    getProductSubCategory(product) ? `التصنيف الفرعي: ${getProductSubCategory(product)}` : '',
    `المتجر: ${brand}`
  ].filter(Boolean);
...
function syncDynamicStructuredData() {
  ...
  const graph = [
    ...
    ...topProducts.map((product) => ({
      ...
      category: product.season || getCodeCategoryLabel(product.codeCategory) || 'أدوات منزلية',
...
function renderCart() {
  ...
  el.cartItemsCount.textContent = `${count} عنصر`;
...
function rebuildSeasonOptions() {
  ...
  el.productSeasonInput.value = options.includes(current) ? current : (options[0] || 'عام');
}
  ...
  products.forEach((product) => {
    ...
    item.innerHTML = `
      ...
      <div>
        ...
        <div class="muted">موديل ${escapeHTML(product.model || '')}${product.season ? ` • ${escapeHTML(product.season)}` : ''}${getProductSubCategory(product) ? ` • ${escapeHTML(getProductSubCategory(product))}` : ''}</div>
...
async function saveProduct() {
  ...
  const season = el.productSeasonInput.value || getSeasonOptions()[0] || 'المطبخ';
  const discountPercent = clamp(toNumber(el.productDiscountInput.value), 0, 99);
  const dozenQtyText = el.productMinQtyInput.value.trim() || '12';
  const dozenQtyNumber = getSeriesQtyNumber({ seriesQtyText: dozenQtyText });
  const pricePiece = rawPricePiece !== '' ? toNumber(rawPricePiece) : (rawPriceDozen !== '' ? round2(toNumber(rawPriceDozen) / Math.max(1, dozenQtyNumber)) : 0);
  const priceWholesale = rawPriceDozen !== '' ? toNumber(rawPriceDozen) : round2(pricePiece * Math.max(1, dozenQtyNumber));
...
function populateProductForm(product) {
  ...
  el.productSeasonInput.value = product.season || getSeasonOptions()[0] || 'المطبخ';
...
function resetProductForm() {
  ...
  el.productSeasonInput.value = getSeasonOptions()[0] || 'المطبخ';
...
async function importProductsExcel(event) {
  ...
  try {
    ...
    rows.forEach((row) => {
      ...
      const seriesQtyText = String(firstValue(row, ['seriesQtyText', 'seriesQty', 'عدد القطع في الدستة', 'كمية السيري', 'السيري', 'minQty', 'الحد الأدنى']) || '12').trim();
      ...
      const seriesPriceRaw = firstValue(row, ['priceWholesale', 'priceSeries', 'priceDozen', 'price', 'سعر الجملة', 'سعر الدستة', 'سعر السيري']);
      ...
      batch.set(ref, {
        ...
        season: String(firstValue(row, ['season', 'الموسم', 'mainCategory', 'القسم الرئيسي']) || getSeasonOptions()[0] || 'المطبخ').trim(),
...
function openQuickProduct(product) {
  ...
  const thumbUrl = getProductThumbUrl(originalUrl);
  const productLabel = product.name || `موديل ${product.model || ''}`.trim() || 'منتج';
  const mainCategoryLabel = product.season || '—';
...
function getDozenBasePrice(product) {
  const explicit = product?.priceWholesale ?? product?.unitPrice ?? product?.priceDozen;
  if (explicit !== undefined && explicit !== null && String(explicit).trim() !== '') return round2(toNumber(explicit));
  return round2(getPiecePrice(product) * getSeriesQtyNumber(product));
}
