...
const APP_SW_VERSION = 'elitehome-production-ultra-v18';
...
const STORE_CACHE_KEYS = {
  products: 'elitehome_cache_products_v18',
  categories: 'elitehome_cache_categories_v18',
  company: 'elitehome_cache_company_v18',
  storefront: 'elitehome_cache_storefront_v18',
  storeSettings: 'elitehome_cache_store_settings_v18',
  payments: 'elitehome_cache_payments_v18',
};
const LIVE_DATA_NOTICE_KEY = 'elitehome_live_data_notice_v18';
...
const el = {
  ...
  mobileNavHome: id('mobileNavHome'),
  mobileNavSearch: id('mobileNavSearch'),
  mobileNavCart: id('mobileNavCart'),
  mobileCartCount: id('mobileCartCount'),
  mobileNavContact: id('mobileNavContact'),
  ...
  cartScreenKindsText: id('cartScreenKindsText'),
...
function bindUI() {
  ...
  bindPress(el.mobileNavMenu, () => openDrawer('menu'));
  bindPress(el.mobileNavCart, () => openDrawer('cart'));
  bindPress(el.mobileNavContact, () => openModal('contactModal'));
  bindPress(el.mobileNavHome, () => {
    closeDrawers();
    goToSeasonsView({ closeAfter: true });
    window.scrollTo({ top: 0, behavior: 'smooth' });
  });
  bindPress(el.mobileNavSearch, () => {
    closeDrawers();
  ...
  el.menuAdminBtn?.addEventListener('click', () => openDrawer('admin'));
...
function subscribeData() {
  onSnapshot(collection(db, 'products'), (snapshot) => {
    ...
    writeStoreCache(STORE_CACHE_KEYS.products, state.products);
    state.cache.visibleProductsSource = null;
    state.cache.filterSignature = '';
    state.cache.filteredProducts = [];
    resetRenderedProducts();
    scheduleRenderEverything();
  }, (error) => handleLiveDataError(error, 'products'));
  onSnapshot(collection(db, 'categories'), (snapshot) => {
    state.categories = snapshot.docs.map((entry) => ({ id: entry.id, ...entry.data() }));
    writeStoreCache(STORE_CACHE_KEYS.categories, state.categories);
    scheduleRenderEverything();
  }, (error) => handleLiveDataError(error, 'categories'));
  onSnapshot(doc(db, 'company', 'main'), (entry) => {
    state.company = { ...DEFAULT_COMPANY, ...(entry.exists() ? entry.data() : {}) };
    writeStoreCache(STORE_CACHE_KEYS.company, state.company);
    scheduleRenderEverything();
  }, (error) => handleLiveDataError(error, 'company'));
  onSnapshot(doc(db, 'settings', 'storefront'), (entry) => {
    state.storefront = { ...DEFAULT_STOREFRONT, ...(entry.exists() ? entry.data() : {}) };
    writeStoreCache(STORE_CACHE_KEYS.storefront, state.storefront);
    applyTheme();
    syncInstallButtons();
    scheduleRenderEverything();
  }, (error) => handleLiveDataError(error, 'storefront'));
  onSnapshot(doc(db, 'settings', 'store'), (entry) => {
    state.storeSettings = normalizeStoreSettings(entry.exists() ? entry.data() : {});
    writeStoreCache(STORE_CACHE_KEYS.storeSettings, state.storeSettings);
    scheduleRenderEverything();
  }, (error) => handleLiveDataError(error, 'store'));
  onSnapshot(doc(db, 'payments', 'default'), (entry) => {
    ...
    writeStoreCache(STORE_CACHE_KEYS.payments, state.payments);
    scheduleRenderEverything();
  }, (error) => handleLiveDataError(error, 'payments'));
  onSnapshot(doc(db, 'shipping', 'default'), (entry) => {
    ...
    writeStoreCache(STORE_CACHE_KEYS.storeSettings, state.storeSettings);
    scheduleRenderEverything();
  }, (error) => handleLiveDataError(error, 'shipping'));
}

function serializeForCache(value) {
  return JSON.parse(JSON.stringify(value, (_, item) => (item && typeof item.toMillis === 'function' ? item.toMillis() : item)));
}

function writeStoreCache(key, value) {
  try {
    saveLocalJSON(key, serializeForCache(value));
  } catch (error) {
    console.warn('Cache write failed', key, error);
  }
}

function preloadCachedStoreState() {
  const cachedProducts = loadLocalJSON(STORE_CACHE_KEYS.products, []);
  if (Array.isArray(cachedProducts) && cachedProducts.length) {
    state.products = cachedProducts.map((entry) => enrichProduct(normalizeProductRecord(entry)));
    state.products.sort((a, b) => toMillis(b.createdAt || b.updatedAt) - toMillis(a.createdAt || a.updatedAt));
  }
  const cachedCategories = loadLocalJSON(STORE_CACHE_KEYS.categories, []);
  if (Array.isArray(cachedCategories) && cachedCategories.length) state.categories = cachedCategories;
  state.company = { ...DEFAULT_COMPANY, ...loadLocalJSON(STORE_CACHE_KEYS.company, {}) };
  state.storefront = { ...DEFAULT_STOREFRONT, ...loadLocalJSON(STORE_CACHE_KEYS.storefront, {}) };
  state.storeSettings = normalizeStoreSettings(loadLocalJSON(STORE_CACHE_KEYS.storeSettings, {}));
  const cachedPayments = loadLocalJSON(STORE_CACHE_KEYS.payments, null);
  if (cachedPayments) {
    state.payments = {
      ...cachedPayments,
      methods: normalizePaymentMethods(cachedPayments.methods, cachedPayments.defaultMethodLabel || 'كاش'),
    };
  }
}

function handleLiveDataError(error, source = '') {
  console.error(error);
  if (sessionStorage.getItem(LIVE_DATA_NOTICE_KEY)) return;
  sessionStorage.setItem(LIVE_DATA_NOTICE_KEY, `${source}:${Date.now()}`);
  showToast('تم فتح آخر نسخة محفوظة لضمان ثبات المتجر على الموبايل');
}
...
function renderCatalogFlow() {
  ...
  if (step === 'seasons') {
    ...
    const subCategoryContent = subCategoryKeys.length
      ? `<div class="prefix-filters-stack"><div class="prefix-filters-bar">${subCategoryChips}</div></div>`
      : `<div class="catalog-inline-note"><i class="fa-regular fa-folder-open"></i><span>لا توجد تصنيفات فرعية داخل هذا القسم حالياً.</span></div>`;

    const title = selectedSubCategory !== 'all' ? selectedSubCategory : (selectedMainCategory || 'المتجر');
    const description = categoryItems.length === 0
      ? `لا توجد منتجات منشورة داخل قسم ${selectedMainCategory} حالياً.`
...
function renderCart() {
  ...
  el.mobileNavCart?.classList.toggle('has-items', count > 0);
  ...
  if (el.cartScreenKindsText) el.cartScreenKindsText.textContent = uniqueItems ? `${uniqueItems} صنف` : '0 صنف';
  el.cartItems.innerHTML = '';
  if (!state.cart.length) {
    el.cartItems.innerHTML = `
      <div class="cart-item cart-item-empty cart-state-card">
        <div class="cart-empty-copy">
          <strong>السلة فارغة</strong>
          <span class="muted">أضف المنتجات ثم أكمل الطلب بسهولة.</span>
        </div>
        <button type="button" class="ghost-btn cart-continue-btn" data-action="continue-shopping"><i class="fa-solid fa-store"></i><span>متابعة التسوق</span></button>
      </div>`;
    el.cartItems.querySelector('[data-action="continue-shopping"]')?.addEventListener('click', () => closeDrawers());
  ...
  state.cart.forEach((item) => {
    ...
    const fullProduct = state.products.find((product) => product.id === item.id);
    const card = document.createElement('div');
    card.className = 'cart-item cart-item-premium';
    card.innerHTML = `
      <div class="cart-item-main">
        <button type="button" class="cart-item-thumb-btn" data-action="preview-image" aria-label="معاينة صورة ${escapeAttr(item.name || 'المنتج')}">
        ...
        <div class="cart-item-copy">
          <div class="cart-item-topline">
            ...
            <button type="button" class="cart-remove-btn" data-action="remove" aria-label="حذف ${escapeAttr(item.name || 'المنتج')} من السلة"><i class="fa-solid fa-trash-can"></i></button>
          </div>
          <div class="cart-item-tags">
            <span class="cart-chip">${escapeHTML(unitLabel)}</span>
            ${unit === 'dozen' ? `<span class="cart-chip cart-chip--soft">${escapeHTML(getSeriesLabel(item))}</span>` : ''}
          </div>
          <div class="cart-item-pricing">
            <span class="muted">${escapeHTML(getUnitPricingLabel(unit))} ${formatCurrency(toNumber(item.unitPrice))}</span>
            <strong>الإجمالي ${formatCurrency(lineTotal)}</strong>
          </div>
        </div>
      </div>
      <div class="cart-item-footer">
        <div class="qty-box qty-box-premium">
          <button type="button" data-action="plus" aria-label="زيادة الكمية"><i class="fa-solid fa-plus"></i></button>
          <span>${item.qty}</span>
          <button type="button" data-action="minus" aria-label="تقليل الكمية"><i class="fa-solid fa-minus"></i></button>
        </div>
        <button type="button" class="ghost-btn cart-open-product-btn" data-action="open-product"><i class="fa-solid fa-up-right-from-square"></i><span>التفاصيل</span></button>
      </div>`;
    ...
    card.querySelector('[data-action="remove"]').addEventListener('click', () => removeCartItem(itemKey));
    card.querySelector('[data-action="preview-image"]').addEventListener('click', () => openGallery(item.imageUrl ? [item.imageUrl] : [placeholderImage(item.name || 'Elite Home Ware')], 0));
    card.querySelector('[data-action="open-product"]').addEventListener('click', () => {
      if (fullProduct) {
        closeDrawers();
        openQuickProduct(fullProduct);
      }
    });
  ...
  if (el.cartScreenKindsText) el.cartScreenKindsText.textContent = `${uniqueItems} صنف`;
  renderCheckoutPreview();
}
...
function handleAuthChange(user) {
  ...
  if (el.menuAdminBtn) {
    el.menuAdminBtn.hidden = !state.isAdmin;
    el.menuAdminBtn.classList.toggle('hidden', !state.isAdmin);
    el.menuAdminBtn.setAttribute('aria-hidden', String(!state.isAdmin));
    el.menuAdminBtn.setAttribute('tabindex', state.isAdmin ? '0' : '-1');
  }
  subscribeOrdersIfAdmin();
}
...
function changeCartQty(productId, delta) {
  ...
  const item = state.cart.find((entry) => (entry.itemKey || buildCartItemKey(entry.id, entry.saleUnit)) === itemKey);
  if (!item) return;
  item.qty += delta;
  if (item.qty <= 0) state.cart = state.cart.filter((entry) => (entry.itemKey || buildCartItemKey(entry.id, entry.saleUnit)) !== itemKey);
  renderCart();
}
...
function removeCartItem(productId) {
  ...
  state.cart = state.cart.filter((entry) => (entry.itemKey || buildCartItemKey(entry.id, entry.saleUnit)) !== itemKey);
...
function syncInstallButtons() {
  const showInstall = Boolean(state.storefront.installEnabled && state.deferredInstallPrompt);
