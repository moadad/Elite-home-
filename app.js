...
const APP_SW_VERSION = 'elite-home-ware-v4';
...
async function searchOrderStatus() {
  ...
  const phone = String(el.trackOrderPhoneInput?.value || '').trim();
  if (!ref || !phone) {
  ...
  try {
    ...
    const matchDoc = snapshot.docs.find((docItem) => String(docItem.data().customerPhone || '').trim() === phone);
    if (!matchDoc) {
