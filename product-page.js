...
function renderProduct(product, ctx) {
  ...
  const mainCategory = product.season || getCodeCategoryLabel(product.codeCategory, ctx.categories);
  ...
  el.season.textContent = subCategory || '—';
...
function renderRelatedProducts(currentProduct, products, brand, categories) {
  const related = products
    ...
    .filter((item) => String(item.season || '') === String(currentProduct.season || '') || String(getProductSubCategory(item) || '') === String(getProductSubCategory(currentProduct) || ''))
    .slice(0, 8);
  ...
  related.forEach((product) => {
    const urls = normalizeImageUrls(product.imageUrls);
    const imageUrl = urls[0] || `${SITE_URL}assets/icon-512.png`;
    const mainCategory = product.season || getCodeCategoryLabel(product.codeCategory, categories);
    const title = product.name || `موديل ${product.model || ''}`.trim();
...
function getPiecePrice(product) { const explicit = product.pricePiece ?? product.piecePrice ?? product.pricePerPiece; if (explicit !== undefined && explicit !== null && String(explicit).trim() !== '') return round2(toNumber(explicit)); const qty = getSeriesQtyNumber(product); const dozenPrice = toNumber(product.priceWholesale || product.unitPrice || 0); return qty > 0 ? round2(dozenPrice / qty) : round2(dozenPrice); }
function getDozenBasePrice(product) { const explicit = product.priceWholesale ?? product.unitPrice ?? product.priceDozen; if (explicit !== undefined && explicit !== null && String(explicit).trim() !== '') return round2(toNumber(explicit)); return round2(getPiecePrice(product) * getSeriesQtyNumber(product)); }
