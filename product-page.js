...
async function boot() {
  ...
  const cachedPayload = loadLocalJSON(PRODUCT_CACHE_KEY, null);
  if (cachedPayload?.product) {
    state.currencySymbol = String(cachedPayload.storefront?.currencySymbol || 'ل.س').trim() || 'ل.س';
    applyBrand(cachedPayload.company || {}, cachedPayload.storefront || {});
    renderProduct(cachedPayload.product, {
      products: Array.isArray(cachedPayload.products) ? cachedPayload.products : [cachedPayload.product],
      categories: Array.isArray(cachedPayload.categories) ? cachedPayload.categories : [],
      company: cachedPayload.company || {},
      storefront: cachedPayload.storefront || {},
    });
  }
  ...
  state.currencySymbol = String(storefront.currencySymbol || 'ل.س').trim() || 'ل.س';
  ...
  saveLocalJSON(PRODUCT_CACHE_KEY, serializeForCache({ product, products, categories, company, storefront }));
...
function formatCurrency(value) {
  ...
  const symbol = String(state.currencySymbol || 'ل.س').trim() || 'ل.س';
